<template>
  <section class="container-fluid">
    <h3 class="pt-5 fw-bold text-center pb-3">Recent Courses</h3>
    <div class="text-center mb-4">
      <span
        v-for="number in linksCourses"
        :key="number"
        class="p-3 rounded-pill"
        >{{ number }}</span
      >
    </div>

    <div
      class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-6 g-4 mb-5"
    >
      <div
        class="col"
        v-for="(element, index) in courses"
        :key="index + 'popular'"
      >
        <div class="card">
          <img :src="element.img" class="card-img-top" alt="element.text" />
          <div class="card-body">
            <h5 class="card-title">
              {{ element.info }}<i class="fas fa-angle-right"></i>
            </h5>
            <p class="card-text">
              {{ element.text }}
            </p>
          </div>
          <div
            class="border-top d-flex justify-content-between align-items-center"
          >
            <div>
              <span v-for="n in element.vote" :key="n">
                <i class="fas fa-star"></i>
              </span>
            </div>
            <div>
              <span
                class="
                  discount
                  text-decoration-line-through text-secondary
                  d-block
                "
                >{{ element.discount }}</span
              >
              <span>{{ element.price }}</span>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div class="text-center mb-5">
      <button class="btn rounded-pill btn-danger">SHOW ALL</button>
    </div>
    <span
      class="d-flex flex-column end-0 top-50 align-items-end position-absolute"
    >
      <i
        v-for="icon in iconsMarket"
        :key="icon"
        :class="icon"
        class="p-2 border"
      ></i>
    </span>
  </section>
</template>

<script>
export default {
  name: "RecentCourses",
  data: function () {
    return {
      linksCourses: [
        "All Category",
        "Business",
        "Design",
        "Development",
        "It&Software",
        "Lyfestyle",
        "Marketing",
        "Office Productivity",
      ],
      iconsMarket: [
        "fas fa-desktop",
        "fas fa-life-ring",
        "fas fa-wrench",
        "fas fa-shopping-cart",
        "fas fa-mobile-alt",
      ],
      courses: [
        {
          img: require("../../img/895786_7b4b_2-272x161.jpg"),
          info: "Development",
          text: "the complete IOS 10 & Swift 3 Developer Course",
          vote: 4,
          discount: "$199,99",
          price: "$100",
        },
        {
          img: require("../../img/1561458_7f3b-272x161.jpg"),
          info: "Design",
          text: "CSS - The Complete Guide 2020(incl. Flexbox,Grid & Sass)",
          vote: 4,
          discount: "$199,99",
          price: "$100",
        },
        {
          img: require("../../img/246154_d8b0_3-272x161.jpg"),
          info: "Development",
          text: "Web Design for beginners:Real World Coding in HTML & CSS",
          vote: 4,
          discount: "$129,99",
          price: "$65",
        },
        {
          img: require("../../img/1208228_d61c_4-272x161.jpg"),
          info: "Design",
          text: "Digitally Painting light and Color:Amateur to Master",
          vote: 4,
          discount: "$139,99",
          price: "$70",
        },
        {
          img: require("../../img/186792_41e4_4-272x161.jpg"),
          info: "Arts & Crafts",
          text: "Android Java Masterclass - Become an App Developer",
          vote: 4,
          price: "$199.99",
        },
        {
          img: require("../../img/1776542_30b1-272x161.jpg"),
          info: "Google",
          text: "Google Searching Ninja",
          vote: 4,
          discount: "$199,99",
          price: "$95",
        },
        {
          img: require("../../img/949316_2a64_11-272x161.jpg"),
          info: "Arts & Crafts",
          text: "Foundations for Mastering Watercolor Painting",
          vote: 4,
          discount: "$19,99",
          price: "$10",
        },
        {
          img: require("../../img/366802_6fcc-272x161.jpg"),
          info: "Food & Beverage",
          text: "get Wine-Smart",
          vote: 4,
          discount: "$49,99",
          price: "$25",
        },
        {
          img: require("../../img/381588_2e6d_4-272x161.jpg"),
          info: "Arts & Crafts",
          text: "The colored Pencil Drawing Corde",
          vote: 4,
          discount: "$34,99",
          price: "$18",
        },
        {
          img: require("../../img/838056_611a_3-272x161.jpg"),
          info: "Arts & Crafts",
          text: "Paint Realistic Watercolor and Botanicals-STUDIO BASICS",
          vote: 4,
          discount: "$69,99",
          price: "$35",
        },
        {
          img: require("../../img/1414956_d944_15-272x161.jpg"),
          info: "Arts & Crafts",
          text: "Mastering Brushstrokes -Part 1",
          vote: 4,
          discount: "$19,99",
          price: "$10",
        },
        {
          img: require("../../img/1109398_4c13-272x161.jpg"),
          info: "Arts & Crafts",
          text: "Paint Realistic Watercolour and Botanicals-MAGNOLIAS",
          vote: 4,
          discount: "$69,99",
          price: "$35",
        },
      ],
    };
  },
};
</script>

<style lang="scss" scoped>
section {
  margin: 0 auto;
  width: 90%;
  .card {
    padding: 0;
    cursor: pointer;
    min-height: 280px;
  }
  .fw-bold {
    padding: 0;
  }
  .card-title {
    font-size: 10px;
    color: lightgray;
  }
  .card-text {
    font-size: 12px;
  }
  .fa-star {
    font-size: 10px;
    color: gold;
  }
  .discount {
    font-size: 10px;
  }
  .border-top {
    padding: 0.5rem;
  }
  span.rounded-pill {
    font-size: 12px;
    color: gray;
  }
  .rounded-pill:hover {
    background-color: lightgray;
    cursor: pointer;
  }
  i {
    min-width: 36px;
    cursor: pointer;
  }
  i:hover {
    background-color: gray;
    color: white;
  }
}
</style>